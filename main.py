...
# theme_sound = pygame.mixer.Sound("Assets/music.mp3")
state = "X turn"
...
def main():
    ...
    while True:
        ...
        if retreat_rect.collidepoint(mouse):
            draw_button("Rematch", (150,150,150), retreat_rect)
            ...
            draw_button("Rematch", (255,255,255), retreat_rect)

        draw_offer_rect = pygame.Rect(600, 200, 100, 50)
        if draw_offer_rect.collidepoint(mouse):
            draw_button("Exit", (150,150,150), draw_offer_rect)
            if pygame.mouse.get_pressed()[0]:  # Kiểm tra xem nút chuột trái có được nhấn không
                # theme_sound.stop()
                TicTacToe.main_menu() 
        else:
            draw_button("Exit", (255,255,255), draw_offer_rect)
